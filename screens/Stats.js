...
export default class Stats extends React.Component {
    ...
    get_deatils = () => {
        ...
        axios
            .post(
                `https://camp-coding.org/ZFactory/select_profits.php`,
                { merchent_id: 1 }
            )
            .then((res) => {
                if (res.status == 200) {
    ...
    getMon(dataRes) {
        ...
        this.setState({ months, orders: orders_dates })
    ...
    collect_orders(mon) {
        let alldata = this.state.orders,
            items = []
        for (let i = 0; i < alldata.length; i++) {
            if (alldata[i].date === mon) {
                items.push(alldata[i].items)
            }
        }
        this.props.navigation.navigate("StateDetils", {
            alldata: items,
            mon_num: mon
        ...
        return (
            <TouchableOpacity
                ...
                onPress={() => {
                    ...
                    // })
                    this.collect_orders(item.months_num)
